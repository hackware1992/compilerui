...
public class SimpleTabLayout extends HorizontalScrollView {
    ...
    public SimpleTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray e = context.obtainStyledAttributes(APPCOMPAT_CHECK_ATTRS);
        final boolean failed = !e.hasValue(0);
        e.recycle();
        if (failed) {
    ...
    void selectTab(final Tab tab, boolean updateIndicator) {
        ...
        if (currentTab == tab) {
            ...
            final int newPosition = tab != null ? tab.getPosition() : Tab
                     .INVALID_POSITION;
            if (updateIndicator) {
                ...
                animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animator) {
                        ...
                        setIndicatorPosition(
                                 (int) (startLeft + (fraction * (targetLeft - startLeft))),
                                 (int)startLeft + Math.round(fraction * (targetLeft - startLeft)));
                    }
                });
